  ...
  return {
    toHex: function toHex(code) {
      ...
      for (j = code.length - 1; j >= 0; j += -1) {
        ...
        if (n == null) {
          console.error("decode error on ".concat(c));
          return null;
        }
    ...
    byNumber: function byNumber(size, buffer) {
      var code, index, j, mask, ref, ref1, str;
      mask = (1 << bits) - 1;
      str = "";

      for (index = j = ref = size * 8, ref1 = -bits; ref1 !== 0 && (ref1 > 0 ? j <= 0 : j >= 0); index = j += ref1) {
        n = index - bits;
